...
- Generation only - this skill does not upload the files or start the worker/Ollama.
  Follow AGENTS.md's "ask before testing": starting the worker or running a real upload
  through the pipeline needs the user's go-ahead first.

## Text notes (current)

```powershell
powershell -File .Codex\skills\generate-test-data\scripts\text\generate-notes.ps1
```
