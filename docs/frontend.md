...
Row split into a disclosure button and `.note-actions` (`Process again` only when a file
exists, `Delete`). A `Bin (n)` section with `Restore` / `Delete forever`. After
`Process again` the list **polls the server every 4 s** until the old version's id
disappears — the worker is a separate process and its SSE does not reach the browser
(same reason in `AssetList`).
