...
> Це особистий конспект власника, свідомо українською — не для агентів. Решта доків
> у `docs/` — англійською.

Список тем, до яких повертаємось окремо. Не документація проєкту — саме черга на
розбір. Стан робіт по коду — у [`docs/`](README.md) (за областями).
...
- [ ] **Presigned URL — це креденшел, а не адреса.** Хто його побачить (історія
      ...
      → *Чому ніщо на боці бекенду — logout, протухла сесія, видалення рядка ассета —
        не зупиняє вже виданий підпис?*
...
- [ ] **Чому бекендний шлях і абстракцію сховища прибрали цілком.** Був `IAssetStorage`
      ...
      → `AssetsController`, `StorageRegistration`
      ...
      → *Прапорець прибрали — чим тепер «вимкнути прямий доступ», якщо припече? (підказка:
        ніяк — рубильника більше нема взагалі; `UploadEnabled` / `DownloadEnabled` теж
        прибрали 2026-09-10)*
